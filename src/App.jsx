...
export default function App() {
  ...
  useEffect(() => {
    if ((results || error) && resultsRef.current) {
      ...
      if (isMobile) {
        // On mobile: always scroll results into view
        resultsRef.current.scrollIntoView({ behavior: 'smooth', block: 'start' })
        setShowScrollHint(false)
      } else {
        // On desktop: show a subtle scroll hint if results are below fold
  ...
  const handleSubmit = async (formData) => {
    ...
    // Scroll to results column immediately on mobile when loading starts
